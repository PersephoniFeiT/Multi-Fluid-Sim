...
public class ClockFace : MonoBehaviour
{
    ...
    public Vector2 handCenter(double theta, Vector2 handSize){
        Vector2 halfSize = handSize / 2f;
        return new Vector2(
            -Mathf.Sin((float)theta) * (halfSize.y - halfSize.x),
            Mathf.Cos((float)theta) * (halfSize.y - halfSize.x)
        );
    }
